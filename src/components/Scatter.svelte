...
<svg {height} {width}>
    ...
    <circle r="3" cx={iris.xScale(item.petalLength)} cy={iris.yScale(item.petalWidth)} fill={iris.colorScale(item.species)} />
    {/each}
    ...
    <g transform={`translate (${width - 100}, ${height - 100})`}>
        {#each iris.species as species, i}
            <g transform={`translate(0 ${i * 20})`}>
                ...
                <text dominant-baseline="middle" x="15" y="5">{species}</text>
            </g>
        {/each}
    </g>
</svg>
